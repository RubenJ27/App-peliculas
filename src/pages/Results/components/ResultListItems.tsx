...
const ResultListItems = ({ id, title, image, onListItemClick }: ResultListItemsData): JSX.Element => {
    ...
    <>
      {
        <div key={id} className="flex flex-row w-full mt-10" style={{ minWidth: 650 }}>
          <div className="w-1/6">
            {image !== undefined ? <img src={image.imageUrl} alt={title} className="w-32" /> : null}
          </div>
